...
from app.services.paywall import decide, is_vip, pay_with_ad, pay_with_coins
...
async def unlock(db: AsyncSession, user_id: str, payload: UnlockRequest) -> UnlockResponse:
    ...
    if source == "ad":
        result = await pay_with_ad(db, user_id, episode_id)
        return UnlockResponse(ok=result["ok"], source="ad", coins_after=result.get("coins_after"), playback_url=result.get("playback_url"))
    if source == "vip":
